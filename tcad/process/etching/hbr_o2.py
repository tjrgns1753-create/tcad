...
"""
HBr/O2 plasma etching (holeEtching example family, same shape as
sf6o2.py's SF6/O2 model).

Real API used (verified against installed ViennaPS 4.6.2 via
`vps.HBrO2Etching.__init__.__doc__`, overload 2 -- identical parameter
names/defaults to SF6O2Etching's flat-float overload):
...
@register
class HBrO2Etch(ProcessStep):
    category = "etching"
    name = "hbr_o2"
    display_name = "HBr/O2 Plasma Etching"
    ...
    def run(self, recipe: Dict[str, Any], output_dir: str) -> Dict[str, Any]:
        ...
        model = module.HBrO2Etching(**model_kwargs)
        ...
        recorder.capture(geometry, "001_hbr_o2_etch")

        final_mesh = Path(output_dir) / "hbr_o2_final"
