...
- Whether terminal-only slash commands (`/plugin`, `/reload-plugins`)
  work is **session-type dependent, not a fixed project fact** — do
  not assume either way without checking the current session type. In
  a **remote/web Claude Code session** (managed cloud container), they
  are confirmed unsupported: `/plugin` and `/reload-plugins` return
  "isn't available in this environment" there (not a permissions
  issue). In a **local desktop-app or local CLI session**, they can
  work: confirmed 2026-09-07, in a local session working this repo,
  `/ponytail:*` and `/claude-md-management:*` slash commands ran
  normally.
...
## Project memory: read `docs/investigation_log.md` and `docs/sdd_completions_log.md`

Claude has no memory between sessions. This file (CLAUDE.md) is kept
short on purpose, and full detail lives in two archive files, split by
how the work was done: `docs/investigation_log.md` holds the full,
session-by-session "what was tested / result / what it proves / next
experiment" writeup for every root-cause investigation this project
has ever run — root causes, rejected hypotheses, exact numbers.
`docs/sdd_completions_log.md` holds the same level of detail for every
plan completed via `superpowers:subagent-driven-development` (SDD) —
full task lists, task/final-review findings, fix-round commits, and
regression numbers — which is NOT duplicated in
`docs/investigation_log.md`, and is NOT recoverable from
`docs/superpowers/plans/*.md` (those record pre-execution intent only;
the SDD skill deletes its own per-plan ledger once a plan's final
review comes back clean, so this log is often the only surviving
execution record). **Before re-investigating anything that sounds even
slightly familiar, grep both files for it — a past session has very
likely already found and fixed it, and re-discovering it wastes a
session.** This CLAUDE.md only carries: the rules above, a one-line-
per-item summary of what's resolved (below, with pointers into
whichever archive has the full record), and full detail on what's
still genuinely OPEN (further below) — since that's what a new session
...
Four fixes, each task-reviewed plus a final whole-branch review (3
Important + 4 Minor findings, fixed in one wave): SiO2 now really
blocks doping (`derive_barrier_covered_windows()`); litho display
staleness tracking (`_litho_pending_since_last_mesh`); an
Oxidation→PR→Etch diagnostic log (`_log_etch_material_summary()`); and
independent donor+acceptor input for all 4 doping kinds. Commits
`06720c4..1e043e2`. Full task list, review findings, and regression
numbers: search `docs/sdd_completions_log.md` for "Litho/doping/renderer
fixes".

### Electrode/pin + DC sweep (SDD, 12 tasks)
CAD-style electrode/pin placement + coordinate-to-mesh-boundary contact
mapping + DC operating point + DC sweep (Id-Vgs, Id-Vds). Plan:
`docs/superpowers/plans/2026-08-27-electrode-pin-dc-sweep.md`. Final
whole-branch review: 1 Critical + 7 Important findings, fixed. Commits
`d66fff4..383889d`. **Known limitation, still true:** the
body-effect-on-Vth is NOT verified anywhere in this project — see
`docs/investigation_log.md`, "DevSim cross-solve sensitivity". Full
task list, review findings, and regression numbers: search
`docs/sdd_completions_log.md` for "Electrode/pin + DC sweep".

### State-dependent process physics — Stage A (SDD, 3 tasks)
First stage of a user-initiated architectural direction: make each
process step's physics genuinely depend on the CURRENT wafer state —
materials, geometry, doping, thermal history — rather than an
interaction-coefficient lookup keyed by process pairs. Design:
`docs/superpowers/specs/2026-09-01-state-dependent-process-physics-design.md`.
Pure plumbing, zero physics-behavior change: `DopantProfile`
(`tcad/physics/dopant_profile.py`) adapts the existing `DopingProfile`/
`DopingRegion` shape; `WaferState` gained `dopant_profiles` +
`donor_concentration_at`/`acceptor_concentration_at`/`net_doping_at`.
Verified at machine-epsilon against real solved DevSim `NetDoping` for
all 4 doping kinds. Final review: 1 Important + 4 Minor findings, fixed.
Commits `ec023f2..586e79e`. Full regression: 72 passed / 3 failed / 0
skipped. Full task list and review findings: search
`docs/sdd_completions_log.md` for "State-dependent process physics —
Stage A".

### State-dependent process physics — Stage B (SDD, 8 tasks)

Second stage: a literature-cited, Arrhenius-based thermal-anneal model
(`tcad/physics/diffusion_model.py` — `arrhenius_diffusivity()`,
`thermal_budget_contribution()`, `anneal_profile()`), grounded in
**Christensen, Radamson, Kuznetsov, Svensson, "Phosphorus and boron
diffusion in silicon under equilibrium conditions", Appl. Phys. Lett.
82(14), 2254-2256 (2003), DOI 10.1063/1.1566464** — P: D0=8e-4 cm²/s,
Ea=2.74±0.07eV, valid 810-1100°C; B: D0=0.06 cm²/s, Ea=3.12±0.04eV,
valid 810-1050°C — still the only real citations in
`INTERACTION_COEFFICIENTS`, and still in production unchanged. Plan:
`docs/superpowers/plans/2026-09-02-state-dependent-physics-stage-b.md`.
Final whole-branch review: 5 Important + 5 Minor findings, all fixed
(commit `c6e7f40`), one scoped re-review verdict CLEAN.

This stage also shipped multi-profile accumulation
(`DopingRegion.gaussian_terms` + `apply_gaussian_implant_doping(...,
existing=...)`), Gaussian-only — **superseded and retired entirely by
the Dopant-state-unification plan below**, which generalized
accumulation to `WaferState.dopant_profiles` for all 4 doping kinds.
`mesh_import.py`'s `_derive_refine_from_doping` auto-refinement is
still single-term-blind (an under-resolved earlier implant's junction)
— explicitly out of scope for both this stage and dopant-state-
unification, tracked as a standalone follow-up, not fixed. Full task
list, review findings, and GUI-observable sensitivity proofs: search
`docs/sdd_completions_log.md` for "State-dependent process physics —
Stage B".
...
mechanism (`ProcessResult.doping` was a per-step declaration, never a
real cross-step state, so process ORDER couldn't genuinely affect a
final device). Design:
`docs/superpowers/specs/2026-09-03-dopant-state-unification-design.md`.
Plan: `docs/superpowers/plans/2026-09-03-dopant-state-unification.md`.

Adds **THE STATE INVARIANT** as a project-level rule alongside THE
INVARIANT above: `WaferState(t) -> Process Physics -> WaferState(t+1)`
— every physical quantity is either preserved (physical-state
preservation, not verbatim data copy) or transformed by real physics
when a step touches it. Central mechanism: `WaferState.net_doping_at(x,
y)` is a real THREE-WAY dispatch — real physics where `host_material`
is exposed; a geometry-gated ZERO for a `"removal"` category (etching);
`UNSUPPORTED_BY_MODEL` — never a silent zero — for `"conversion"`
(oxidation) or any unclassified category. `WaferState.dopant_profiles`
is now the one real cross-step canonical doping state (any doping kind,
via `advance_wafer_state()`), replacing Stage B's Gaussian-only
`gaussian_terms`. Three real ViennaPS/DevSim acceptance tests
(`test_ce1_order_sensitive_geometry_real.py`,
`test_ce2_oxidation_conversion_unsupported_real.py`,
`test_ce3_implant_anneal_etch_implant_real.py`) prove the architecture,
strictly x-only.

Final whole-branch review (Opus, 4 passes): 9 Important findings — the
real `UNSUPPORTED_BY_MODEL` disclosure contract was enforced rigorously
inside `WaferState`, then quietly eroded at every boundary it crossed
on the way to a user — all fixed in one wave (commit `7854c85`), one
scoped re-review, 1 residual Minor fixed directly (commit `56c492f`).
Full regression after the fix wave: **80 passed / 3 failed / 0
skipped**, identical 3 pre-existing DevSim-convergence failures as
every prior run, zero new failures. Two real, disclosed limitations
this plan does NOT solve are carried forward as OPEN items 4 and 5
below (`exposed_material_at()`'s no-y-awareness / no GUI-overlay
rescue; `last_step_category`'s flat, non-per-location provenance).

Full task list, per-task fix rounds, the complete final-review findings
list, and every commit/regression number: search
`docs/sdd_completions_log.md` for "Dopant-state-unification".

### LOCOS split out of ThermalOxidation into its own Advanced/Optional process

User-initiated: LOCOS must never be a core feature. Split the previously
fused `ThermalOxidation` implementation into two independent, sibling
`ProcessStep`s — `tcad/process/oxidation/thermal.py` (`ThermalOxidation`,
registry `("oxidation","thermal")`, CORE, plain fin-style, no mask
concept at all) and `tcad/process/oxidation/locos.py`
(`LocosOxidation`, registry `("oxidation","locos")`, ADVANCED/OPTIONAL,
501 lines, all LOCOS physics/geometry moved verbatim, not rewritten).
GUI now genuinely dispatches a different `_process_model_key` per
selection — a real, pre-existing bug was found and fixed along the way:
`tcad_2d_stagewise.py` sent `"_process_model_key": "thermal"`
unconditionally for BOTH combobox choices, so selecting "LOCOS" in the
GUI silently ran plain thermal oxidation. 8 test files were migrated
...
existing custom `save_locos_volume_mesh()` per the user's own checklist
(materials, node/cell counts, floor clipping, DevSim ingestion,
contact/NetDoping mapping, process chaining). Verdict:
**NEEDS-FURTHER-VALIDATION, current custom exporter KEPT, not
removed** — the native call alone has no floor treatment (Si clips to
2x gridDelta) and a fresh/unconfigured model instance silently drops
the Mask material entirely (a false-negative found and self-corrected
mid-investigation); combined with this project's own
`_floored_copy_for_export()` and the real, already-configured model
instance, areas match the custom exporter within 0.04-2.30% and import
successfully through the real DevSim pipeline — promising, but not
validated deeply enough across topologies to justify replacing a
working exporter.

**Found, deliberately NOT fixed this session (two real, reproducible
chaining failures, both in shared infrastructure outside LOCOS's own
scope, per the user's own priority ordering):** LOCOS chained onto a
domain built by a FOREIGN prior step (not a prior LOCOS call) can fail
two different ways — a real ViennaPS internal inconsistency during the
oxidation solve itself (`no oxide nodes found` alongside a garbage
~10^308 displacement value) after a deposition+remask chain, and a
`IndexError: invalid vector subscript` inside
`_floored_copy_for_export()`'s own boolean-intersect step after a
simpler bare-Si+remask chain (the existing `vls.Expand(ls, 3)`
pre-treatment does not cover this topology). Full repro recipes,
...
note in "Resolved investigations" below for that finding's own
writeup). Full A-F formal report, the complete separation/physics-
validation/exporter-comparison tables, and the exact per-file line
...
92-total reference are the new `test_locos_birds_beak_real.py` plus
suite growth already in flight before this session; the pass/fail
identity of every pre-existing test is otherwise unchanged.
...
**A 4th failure is not automatically this flake class.** Do not classify
a fourth failure as the existing DevSim/solver-noise flake without
checking the test name and actual failure message — in particular, a
mock/unit-test failure can never be solver noise at all. See
`docs/investigation_log.md` for the LOCOS regression investigation this
lesson came from.
