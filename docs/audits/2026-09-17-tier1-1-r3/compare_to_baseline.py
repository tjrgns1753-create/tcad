"""Tier 1-1 r3: derive BOTH required reports from the ONE full-audit run
...
def main():
    ...
    if not new_data["complete"]:
        raise SystemExit(f"r3 audit not finished: {len(new_all)}/{new_data['planned']}")
    ...
    lines = ["# Tier 1-1 r3 -- 120개 baseline cohort 대비 (2026-09-16)", "",
             f"- baseline: {json.dumps(s_base, ensure_ascii=False)}",
             f"- r3 cohort (같은 120개, 이번 감사에서 추출): {json.dumps(s_cohort, ensure_ascii=False)}", "",
    ...
    lines2 = ["# Tier 1-1 r3 -- 전체 현재 테스트 (baseline 120 + 신규)", "",
              ...
              "## 기존(baseline) 실패 지속", "", "| 테스트 | baseline | r3 | 분류 |", "|---|---|---|---|"]
    ...
    if not existing_fail:
        lines2.append("| (없음) | | | |")
    lines2 += ["", "## baseline에 없던 신규 실패 (120개 중, r3에서 새로 실패)", "",
               "| 테스트 | baseline | r3 | 분류 | 비고 |", "|---|---|---|---|---|"]
    for test in baseline_names:
        b, n = base[test], cohort[test]
    ...
    print(json.dumps(dict(
        baseline=s_base, r3_cohort_120=s_cohort, r3_full=s_full,
