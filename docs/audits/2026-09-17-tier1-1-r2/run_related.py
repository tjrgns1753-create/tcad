"""Tier 1-1 supplement, step 2: related existing tests, serially, same
environment as the 120-file audit. Every unit test plus the integration
tests that reach apply_doping / the robust sweep / a GUI measurement.
Writes logs + results.json to ./related/.
"""
import json
import os
import subprocess
...
print("SUMMARY", {s: sum(r["status"] == s for r in rows) for s in sorted({r["status"] for r in rows})})
