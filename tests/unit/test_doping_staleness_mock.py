...
def main():
    ...
    try:
        ...
        def _fake_apply_uniform_doping(process_result, donor_by_region_cm3=None,
                                       acceptor_by_region_cm3=None,
                                       chemical_state=None, **kw):
            (name, donor), = donor_by_region_cm3.items()
            result = _Stub()
            result.volume_mesh_path = app.last_final_mesh
            result.material_regions = []
            # A REAL DopingProfile carrying the declaration run_doping()
            # passed (donor and acceptor separate, state explicit).
            result.doping = DopingProfile(
                kind="uniform",
                regions=[DopingRegion(
                    region=name, net_doping_cm3=donor - acceptor_by_region_cm3[name],
                    donor_conc_cm3=donor, acceptor_conc_cm3=acceptor_by_region_cm3[name],
